...
1. **RetrieveScheduleResponse Subcommands:** If `0x20` is somehow miraculously still used, what are the new hex boundaries for determining Program Detail vs Start Time vs Run Time when program totals exceed 15?
1. **Queue Payload Format:** For the unimplemented `CurrentQueueRequest` (`0x3B`), how exactly does the app decode the `BB` response? What does the array of delayed/queued programs and stations look like?
1. **Seasonal Adjust Array Dimensions:** Does `ZonesSeasonalAdjustFactorResponse` (`0xB2`) always span exactly 32 stations (16 pairs of bytes), or does the response dynamically scale or require multi-paging depending on the active station count?
