...
1. **Authentication:** The client prompts the user for cloud credentials (username/password) and calls `async_authenticate_cloud`.
   ...
   - **Official App Behavior:** The decompiled official Android app leverages the **AWS Amplify SDK** (integrated with Cognito and AppSync). It launches an Android Custom Tab (or System Web View) to handle the user login over the `/coreidentityserver/connect/authorize` endpoints. Upon successful login, the OIDC identity server redirects to the authorized redirect URI, which the app intercepts via deep-linking/intent filters to capture the JWT tokens. The app then persists the tokens locally in its encrypted SharedPreferences and injects them dynamically into outgoing request headers using an `OidcAuthProvider` HTTP header interceptor.
   - **Programmatic Emulation:** Because the Rain Bird identity server strictly restricts redirection to `https://iq4.rainbird.com/auth.html` and does not support registering arbitrary external redirect URIs (e.g. local home automation domains), standard browser redirection cannot be used by third-party integrations.
   - **Mechanism:** Instead, `async_authenticate_cloud` programmatically performs these same OAuth steps: it initiates the authorization flow, parses the CSRF verification token from the login page, POSTs the credentials, and extracts the JWT `access_token` from the resulting redirect URL hash fragment.
